...
import { addBulkToQueue, processQueue } from "../src/lib/queue";
...
function buildTemplateComponents(template: { variables?: string | null }) {
...
async function resolveCredential(orgId: string): Promise<string> {
...
async function resolveContact(orgId: string, phone: string) {
